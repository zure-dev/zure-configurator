...
import {
  ...
  InlineStack, BlockStack, Text, Badge, Spinner, Tabs, Divider,
...
export default function PricingEditorPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const groupsList = optionsData.optionGroups ?? [];
      setGroups(groupsList);

      const mapRules = (rules: any[]) => rules.map((r: any) => {
        const group = groupsList.find((g: any) => g.slug === r.optionGroupSlug);
        const value = group?.values?.find((v: any) => v.slug === r.optionValueSlug);
        return {
  ...
  const handleSaveRetail = useCallback(async () => {
    ...
    setErrorMsg('');
    try {
      const rules = retailRules.filter((r) => r.priceModifier !== '0' && r.priceModifier !== '').map((r) => ({
        optionGroupSlug: r.optionGroupSlug,
        optionValueSlug: r.optionValueSlug,
      ...
      if (!res.ok) { setErrorMsg('Failed to save'); return; }
  ...
  const handleSaveTrade = useCallback(async () => {
    ...
    setErrorMsg('');
    try {
      const rules = tradeRules.filter((r) => r.priceModifier !== '0' && r.priceModifier !== '').map((r) => ({
        optionGroupSlug: r.optionGroupSlug,
        optionValueSlug: r.optionValueSlug,
        priceModifier: parseFloat(r.priceModifier),
        modifierType: r.modifierType,
  ...
  // Build a complete grid of all group/value combos
  const buildFullGrid = (existingRules: PriceRuleRow[]): PriceRuleRow[] => {
    const grid: PriceRuleRow[] = [];
    for (const group of groups) {
      for (const value of group.values ?? []) {
        const existing = existingRules.find(
          (r) => r.optionGroupSlug === group.slug && r.optionValueSlug === value.slug
        );
        grid.push(existing ?? {
  ...
  if (loading) {
    return (
      <Page title="Pricing" backAction={{ content: 'Back', url: `/admin/product-families/${familyId}` }}>
  ...
  const renderPriceGrid = (grid: PriceRuleRow[], setRules: (fn: (prev: PriceRuleRow[]) => PriceRuleRow[]) => void) => {
    let currentGroup = '';
        ...
        <div key={`${row.optionGroupSlug}-${row.optionValueSlug}`}>
          {showHeader && (
            <div style={{ marginTop: i > 0 ? 16 : 0, marginBottom: 8 }}>
              <Text as="h3" variant="headingSm">{row.groupName}</Text>
            </div>
          )}
          <InlineStack gap="300" blockAlign="center">
            <div style={{ width: 180 }}>
              <Text as="span" variant="bodyMd">{row.valueName}</Text>
            </div>
            <div style={{ width: 120 }}>
              <TextField
                label=""
                ...
                onChange={(v) => {
                  setRules((prev) => {
                    ...
                    const idx = next.findIndex(
                      (r) => r.optionGroupSlug === row.optionGroupSlug && r.optionValueSlug === row.optionValueSlug
                    );
                    if (idx >= 0) next[idx] = { ...next[idx]!, priceModifier: v };
                    else next.push({ ...row, priceModifier: v });
                    return next;
                  });
                }}
                prefix="$"
            ...
            <div style={{ width: 100 }}>
              {parseFloat(row.priceModifier) > 0 && <Badge tone="success">+${row.priceModifier}</Badge>}
              {parseFloat(row.priceModifier) < 0 && <Badge tone="critical">${row.priceModifier}</Badge>}
              {parseFloat(row.priceModifier) === 0 && <Badge>Base</Badge>}
            </div>
          </InlineStack>
        </div>
      );
              ...
              <Card>
                <BlockStack gap="300">
                  <InlineStack align="space-between">
                    <Text as="h2" variant="headingMd">Retail Price Modifiers</Text>
                    <Button variant="primary" onClick={handleSaveRetail} loading={saving}>Save Retail Pricing</Button>
                  </InlineStack>
                  <Text as="p" variant="bodySm" tone="subdued">
                    Set the price adjustment for each option value. Positive values add to the base price, negative values subtract.
                  </Text>
                  <Divider />
            ...
            <Layout.Section>
              <Card>
                <BlockStack gap="300">
                  <InlineStack align="space-between">
                    <BlockStack gap="100">
                      <Text as="h2" variant="headingMd">Trade Price Modifiers</Text>
                      <Text as="p" variant="bodySm" tone="subdued">
                        Applied when customer has the "trade" tag. Leave at $0 to use retail pricing for that option.
                      </Text>
                    </BlockStack>
                    <Button variant="primary" onClick={handleSaveTrade} loading={saving}>Save Trade Pricing</Button>
                  </InlineStack>
                  <Divider />
